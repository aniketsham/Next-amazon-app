...
export default function useCartService(){
    ...
    return{
        ...
        increase:(item:OrderItem)=>{
            ...
            ? items.map((x)=>x.slug === item.slug ? {...exist,qty:exist.qty+1} : x):[...items,{...item,qty:1}]
        ...
        decrease:(item:OrderItem)=>{
            ...
            exist.qty==1?items.filter((x:OrderItem)=>x.slug!==item.slug)
        ...
        init:()=>cartStore.setState(initalState)
    }
}
